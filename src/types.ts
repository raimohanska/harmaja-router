...
type ParamsObjectFrom<Parts> = Record<ParamsFrom<Parts>, string>;

export type PathParams<S extends string> = ParamsObjectFrom<Split<S>>;
type ValidPathPart<PartKey extends string> = PartKey extends `:${infer Param}`
    ? string
    : PartKey;

type ValidPathFrom<Parts> = Parts extends [
    infer First,
    infer Second,
    ...infer Rest
]
...
export type RouteMap<R> = {
    [Path in keyof R]: Path extends string
